...
%Therefore, the relationship defined by \texttt{compare} ($\leq$) forms a semi-lattice $L$ on all possible states of an $\infty$P-Set.	
	...
	\step{label-monotonicity}{\textit{(Monotonicity)} All operations that result in a new state $D''$, do so such that $D''$ is either equal or larger than state input(s) passed as arguments in $\mathds{L}$, if any.}
	\begin{proof}
	...
	\qedstep
	\begin{proof}
		\pf By definition, \stepref{label-ordering}, \stepref{label-lub}, \stepref{label-monotonicity} imply that $\infty$P-Set is a state-based CRDT. See Appendix~\ref{sec:longer-proofs:convergence} for more detailed steps.
	\end{proof}
\end{proof}
...
\begin{proof}
	...
	\begin{proof}
		...
		\assume{\begin{pfenum}
			...
			\item $\subseteq$ is a partial order on $\mathcal{P}(E)$ forming a semi-lattice $\mathds{L}_S$.
		\end{pfenum}}
		\prove{$S'' = S \cup S'$ is the least upper bound of $S$ and $S'$ in $\mathds{L}_S$.}}
		\begin{proof}
