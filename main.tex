...
The full behaviour of an $\infty$P-Set implies the followings, starting from an initial state $S$ and two operations $\textit{add}_r(e)$ and $\textit{remove}_{r'}(e)$ respectively happening on replicas $r$ and $r'$ and a final state $S'$ after both replicas $r$ and $r'$ have merged:
\begin{enumerate}
	...
	\item else if $\textit{remove}_r'(e)$ happens before $\textit{add}_{r}(e)$ ($\textit{remove}_{r'}(e) \rightarrow  \textit{add}_r(e)$) then eventually $e \in S$ on $r$ and $r'$ regardless of the state of $e$ in $S$;
	\item else $add_r(e) ~||~ remove_{r'}(e)$ and the result depends on $S$:
	    \begin{enumerate}
...
If replicas $r$ and $r'$ are in different states, there still exists a common state $S$ they both were in in the past (the initial state if nothing else), and from that point, the longest alternating sequence of \textit{add(e)} and \textit{remove(e)} following the causal history across replicas determines the final state.
...
\subsubsection{Known Anomalies Are Avoided}

We show here that anomalies previously identified~\cite{bieniusa:hal-00769554} are correctly handled by $\infty$P-Set. 

First, the specific execution in which the effect of a \textit{remove} is ignored even if it happened after an \textit{add} from a different replica (Fig. 1(a) in~\cite{bieniusa:hal-00769554}) does not happen with $\infty$P-Set because as mentioned previously, in the presence of a causal relationship the remove will apply.
...
\begin{theorem}
Merging any two $\infty$P-Sets $S$ and $S'$, whose last operation was an \textit{add(e)} (or \textit{remove(e)}), always results in a set $S''$ such that $e \in S''$ (respectively $e \notin S''$).
\end{theorem}

\begin{proof}
When observing the corresponding dictionaries $D$, $D'$, and $D''$, with both $D[e]$ and $D'[e]$ are odd because their last operation was an \textit{add(e)}:
\begin{enumerate}
    \item $D[e]=D'[e]$ and after a merge $D''[e]=D[e]=D'[e]$;
    \item $D[e]>D'[e]$ or $D[e]<D[e']$: after a merge $D''[e]=max(D[e], D''[e])$
\end{enumerate}   

In both cases $D''[e]$ is also odd and $e \in S''$.  The proof for \textit{remove(e)} being the last operation of both sequences is similar, except that $D$ and $D'$ are even.
\end{proof}

Third, whenever two replicas $r$ and $r'$ issue the same sequence of \textit{add(e)} and \textit{remove(e)} starting from the same state $S$ (Fig. 1 b and c in~\cite{bieniusa:hal-00769554}), $r$ and $r'$ will both end in the same state $S'$ after synchronization, regardless at which point the synchronization happens and even if no synchronization happens, because the same sequence will result in the same counter for $e$ on all replicas.
...
We plan to extend the design to arbitrary (Byzantine) failures in the future and apply the design in application to validate whether the currency semantics can suit both programmers and end users.
